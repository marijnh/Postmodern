...
;;     client-first -  The client-first message consists of a gs2-cbind-flag, the desired username, and a randomly generated client nonce cnonce.
;;     server-first -  The server appends to this client nonce its own nonce snonce, and adds it to the server-first message, which also contains
;;                     a salt used by the server for salting the user's password hash, and an iteration count indicator it.
;;     client-final -  After that the client sends the client-final message, which contains c-bind-input, the concatenation of the client and the
;;                     server nonce, and cproof.
;;     server-final -  The communication closes with the server-final message, which contains the server proof sproof.
...
;;     where the XOR operation is applied to byte strings of the same length, H(ckey) is a normal hash of ckey. 'Client Key' and 'Server Key' are verbatim strings.

;;     Stored password - The stored password is equal to H(ckey). In the algorithm above, the client proves knowledge of ckey, which is then
;;     hashed and compared against what is stored on the server.
...
It returns the server-response as a normal string, the server-provided-salt as a normal string, and the server-iterations as an integer"
  (cond ((eq response-type :base64-usb8-array) nil)
        ((eq response-type :utf8-string) (setf response (cl-base64:base64-string-to-usb8-array (cl-base64:string-to-base64-string response))))
        ((eq response-type :base64-string) (setf response (cl-base64:base64-string-to-usb8-array response)))
        (t (cerror "Invalid response type for parse-scram-server-first-response. Must be one of :base64-usb8-array, :utf8-string or :base64-string"
                   response-type)))

  (let* ((split-response (split-server-response response))
         (server-nonce (cdr (assoc "r" split-response :test 'equal)))
         (server-nonce-validated (validate-server-nonce server-nonce client-nonce))
...
(defun aggregated-gen-final-client-message (user-name client-nonce server-message password
                                            &key (response-type :base64-usb8-array) (salt-type :base64-string))
...
The allowed response-types are :base64-string, :base64-usb8-array and :utf8-string."
  (multiple-value-bind (server-nonce server-salt server-iterations)
      (parse-scram-server-first-response server-message client-nonce :response-type response-type)
    (let* ((final-message-part-1 (gen-final-message-part-1 server-nonce))
           (client-initial-response (gen-client-initial-response user-name client-nonce))
           (salted-password (gen-salted-password password server-salt server-iterations :salt-type salt-type))
           (client-key (gen-client-key salted-password))
           (stored-key (gen-stored-key client-key))
           (auth-message (gen-auth-message client-initial-response server-message final-message-part-1))
           (client-signature (gen-client-signature stored-key auth-message))
           (client-proof (gen-client-proof client-key client-signature))
           (final-client-message (gen-final-message final-message-part-1 client-proof))
           (server-key (get-server-key salted-password))
           (server-signature (cl-base64:usb8-array-to-base64-string (get-server-signature server-key auth-message))))
