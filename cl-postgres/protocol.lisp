  ...
  (let ((socket-name (gensym))
    ...
    (flet ((expand-characters (chars)
      ...
      `(let* ((,socket-name ,socket)
              (,char-name (read-uint1 ,socket-name))
              (,size-name (read-uint4 ,socket-name)))
        (declare (type (unsigned-byte 32) ,size-name)
                 (type (unsigned-byte 8) ,char-name)
                 (type stream ,socket-name))
        (declare (ignorable ,size-name))
        (case ,char-name
          (#.(char-code #\E) (get-error socket))
          (#.(char-code #\N) (get-warning socket))
          ,@(mapcar (lambda (clause)
                      `(,(expand-characters (first clause)) ,@(if (eq (second clause) :skip)
                                                                  `((skip-bytes ,socket-name (- ,size-name 4)))
                                                                  (cdr clause))))
                     clauses)
          ,@(unless t-found `((t (close ,socket-name)
                               (error 'protocol-error :message (format nil "Unexpected message received: ~A"
                                                                       (code-char ,char-name)))))))))))
