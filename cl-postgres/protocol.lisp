...
(defun authenticate (socket conn)
  ...
  (flet ((add-restart (err)
           (restart-case (error 'malformed-proof-in-scram-client-final-message :scram-error-code err)
             (:re-authenticate () :report "Some kind of malformed client proof in scram" (authenticate socket conn)))))
    (handler-case
        (let ((gss-context nil)
                   ...
                   (scram-msg-cont (in-buffer)
                     (multiple-value-bind (cont-message calculated-server-signature)
                       ...
                       (setf expected-server-signature calculated-server-signature)
                                        ;                 (v:log :info :test "client-nonce ~a server-sig ~a length ~a" client-nonce calculated-server-signature (length cont-message))
          ...
          (loop
             (message-case socket
               ...
               (#\Z (read-uint1 socket)
                    (return)))))
      (scram-authentication-error (e) (add-restart e))))
      socket)
