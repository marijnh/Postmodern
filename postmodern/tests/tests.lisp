...
(test rename-table-and-columns
  (with-test-connection
    ...
    (rename-table 'test-schema.t1 'test-schema.t2)
    (is (table-exists-p 'test-schema.t2))
    (rename-table 'test-schema.t2 't3)
    (is (table-exists-p 'test-schema.t3))
    (is (column-exists-p 'test-schema.t3 'id))
    (rename-column 'test-schema.t3 'id 'new-id)
    (is (column-exists-p 'test-schema.t3 'new-id))
    (rename-column 'test-schema.t3 "new_id" "id")
    (is (column-exists-p "test-schema.t3" "id"))
    (rename-table "test-schema.t3" "t2")
    (is (table-exists-p "test-schema.t2"))
