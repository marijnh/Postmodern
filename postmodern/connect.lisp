...
(defparameter *default-use-ssl* :no "The default for connect's use-ssl argument. This starts at :no. If you set it to anything else, be sure to also load the CL+SSL library.")

(defun connect (database-name user-name password host &key (port 5432) pooled-p (use-ssl *default-use-ssl*) (service "postgres"))
  ...
  (cond (pooled-p
         (let ((type (list database-name user-name password host port use-ssl)))
           (or (get-from-pool type)
               (let ((connection (cl-postgres:open-database database-name user-name password host port use-ssl)))
		 ...
		 #+genera (progn
                 ...
                 connection))))
        (t (cl-postgres:open-database database-name user-name password host port use-ssl service))))
...
(defun connect-toplevel (database-name user-name password host &key (port 5432) (use-ssl *default-use-ssl*))
  ...
  (setf *database* (connect database-name user-name password host :port port :use-ssl use-ssl))
  (values))

(defgeneric disconnect (database)
  (:method ((connection database-connection))
    (close-database connection))
  (:documentation "Disconnects a normal database connection, or moves a pooled connection into the pool."))
...
(defgeneric reconnect (database)
  ...
  (:documentation "Reconnect a disconnected database connection. This is not allowed for pooled connections ― after they are disconnected they might be in use by some other process, and should no longer be used."))
