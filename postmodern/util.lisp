...
(defun load-extension (extension)
  "Loads the Postgresql contrib module or extension provided as a parameter.
The parameter provided must be a string. If the extension is not available,
Postgresql will throw an error. It will be skipped if it is already loaded."
  (setf extension (to-sql-name extension :literal))
  (query (format nil "CREATE EXTENSION IF NOT EXISTS ~a" extension)))
