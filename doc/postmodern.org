...
At the heart of Postmodern's DAO system is the dao-class metaclass. It allows
you to define classes for your database-access objects as regular CLOS classes.
Some of the slots in these classes will refer to columns in the database. To
specify that a slot refers to a column, give it a :col-type option containing an
S-SQL type expression (useful if you want to be able to derive a table
...
and :keys to provide a set of primary keys for the table. If more than one key
is provided, this creates a multi-column primary key and all keys must be
...
Select the DAO object from the row that has the given primary key values, or NIL
...
#+BEGIN_SRC lisp
  (LAMBDA ()
    (LET ((*TABLE-NAME* "country_d") (*TABLE-SYMBOL* 'COUNTRY-D))
      (DOLIST
          (STAT
           (LIST (!DAO-DEF) (!FOREIGN 'REGION-D 'REGION-NAME 'NAME)))
        (EXECUTE STAT))))
#+END_SRC
*** Normalized Tables and Daos
   :PROPERTIES:
...
Question - is region-id nullable? We did not set it up that way. How do you find
the region-id? Remember we set the primary key as name for both country and
region? Lets start by inserting a couple of regions and we will stick with the
dao method for the moment:
#+BEGIN_SRC lisp
...
#+END_SRC
Now we can add a few countries to country-n:
#+BEGIN_SRC lisp
(insert-dao
  (make-instance 'country-n
                 :name "The Netherlands"
                 :region-id (region-id
                              (get-dao 'region "Western Europe"))))

(insert-dao
  (make-instance 'country-n
                 :name "Croatia"
                 :region-id (region-id
                              (get-dao 'region "Southern Europe"))))
#+END_SRC

Now, how to get back the region dao with the name "Western Europe"? For region-d
...
You could, by creating and enforcing rules on how tables and columns are named,
create readers and writers that would automagically handle the joins. Postmodern
is not going to define what those rules are going to be.
...
Return a list of the tables in the public schema of a database. By default the
table names are returned as keywords. They will be returned as lowercase strings
if strings-p is true.
...
Returns a list of the fields in the named table. Each field is represented by a
list of three elements: the field name, the type, and a boolean indicating
