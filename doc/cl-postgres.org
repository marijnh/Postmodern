...
If parameters are not passed and =*use-binary-parameters*= is true, Postgresql will assume the parameters will be text. In order to pass integer, float or boolean parameters as binary while
=*use-binary-parameters*= is true, you need to pass a list of parameters with the same
