...
- [[file:postmodern.html][Postmodern]] itself is the library that tries to put all these things together into a convenient programming interface.
...
establishes the connection locally. If the Postgresql server is running on a port other than 5432,
...
using s-sql rather than plain vanilla sql would be the following:
#+BEGIN_SRC lisp
